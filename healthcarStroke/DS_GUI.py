...
hypertension_dict = {hypertension_index: index for index, hypertension_index in enumerate(list.hypertension_list)}
heart_disease_dict = {heart_disease_index: index for index, heart_disease_index in enumerate(list.heart_disease_list)}
ever_married_dict = {ever_married_index: index for index, ever_married_index in enumerate(list.ever_married_list)}
work_type_dict = {work_type_index: index for index, work_type_index in enumerate(list.work_type_list)}
Residence_type_dict = {Residence_index: index for index, Residence_index in enumerate(list.Residence_type_list)}
smoking_status_dict = {smoking_status_index: index for index, smoking_status_index in enumerate(list.smoking_status_list)}
...
# GUI
st.title("Stroke State With DST")
...
# Predict salary
if st.button("Stroke State With DST"):
    # gender-	age-	  hypertension-	 heart_disease-	 ever_married-	 work_type-	Residence_type	avg_glucose_level	bmi	  smoking_status
    ...
    # stroke state
    stroke_state = dst_modle.predict(scaled_data)[0]
